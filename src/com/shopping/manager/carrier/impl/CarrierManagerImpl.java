package com.shopping.manager.carrier.impl;
...
public class CarrierManagerImpl extends BaseManager implements CarrierManager {
			...
			template.execute(new TransactionCallbackWithoutResult() {
				...
				protected void doInTransactionWithoutResult(
					...
					try {
						...
						if (carrier != null) {
							carrier.setCarrierCode(DateUtil.getSerialNumber());		//set carrier code
							carrier.setCreateTime(new Date());		//set create time
							carrier.setUpdateTime(new Date());		//set update time		
							carrier.setYn(CommonConstant.YN_NO_DELETE);		//set yn=0
	...
	@Override
	public PageModel<Carrier> queryCarrierListPageByQueryBean(CarrierQuery carrierQuery,
		...
		// 创建翻页集合,根据第几页和每页大小
		PageModel<Carrier> carriers = new PageModelImpl<Carrier>(pageIndex,
				pageSize);
