...
<script>
    ...
    function removeFromCart(id){
    	 if (confirm("确认删除该商品?")) {
    		...
    		jQuery.ajax({
				type : "post",
				url : "shoppingCartJson_deleteShoppingCartById.action",
				dataType : 'text',
				data : {id:id},
				success : function(json) {
					...
					if (json != null && json != '') {
						...
						if (eval(jsons[0])!=null) {
							$("#totalPrice").html("￥"+eval(jsons[0]));
						}
					} else {
						alert("异步处理异常，返回值为空！");
					}
    ...
    function updateCommodityNum(id){
    	var commodityNum=$("#commodityNum").val();
    	
    	if(id.length>0&&id!=''&&commodityNum.length>0&&commodityNum!=''){
    		jQuery.ajax({
				type : "post",
				url : "shoppingCartJson_updateShoppingCartCommodityNum.action",
				dataType : 'text',
				data : {id:id,commodityNum:commodityNum},
				success : function(json) {
				...
				<div class="shoppingcart">
					...
					<s:if test="shoppingCartList.size>0">
						...
						<div class="carttable">
							...
							<a href="#" onclick="addFavorites()" class="buttonone right">收藏</a>
							<div class="clear"></div>
							<ul class="head colr">
							...
							<s:iterator value="shoppingCartList" status="status">
								<s:if test="#status.odd">
									<ul class="cartcontents" id="<s:property value="id"/>">
													...
													<s:generator separator=";" val="commodityPhoto">
														<s:iterator status="status">
															<s:if test="#status.index==0">
																<img src="../commodityImages/<s:property/>" width="60"
																	height="60" />
															</s:if>
										...
										<li class="qty"><input id="commodityNum"
											name="commodityNum" type="text"
											value="<s:property value="commodityNum"/>"
											onchange="updateCommodityNum('<s:property value="id"/>')" />
										</li>
						...
						<div class="buttonssec">
							<div class="right">
								<p class="colr bold">
									总计 : <span id="totalPrice">￥<s:property
											value="totalPrice" />
									</span>
								</p>
							</div>
							<div class="right">
								<a href="index.action" class="buttonone">继续购物</a> <a
									href="toCheckOut.action" class="buttonone">结算</a>
							</div>
						</div>
